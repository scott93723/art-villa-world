...
function applyTOD(t) {
  ...
  updateEnv(t);
  const names = [[0.05, '夜晚'], [0.19, '黎明'], [0.42, '清晨'], [0.6, '正午'], [0.78, '午后'], [0.9, '金色时刻'], [0.955, '黄昏'], [1.01, '夜晚']];
...
const VIEWS = [
  ...
  { n: '西南花园', p: [-20, 7.5, 21], t: [-1.5, 3.0, 3.5] },
  { n: '南立面', p: [2.5, 6.0, 35], t: [0.8, 3.4, 3.0] },
  { n: '露台层', p: [-2.2, 1.72, 9.8], t: [3.6, 2.1, 4.6] },
  { n: '客厅', p: [-7.6, 1.60, 3.2], t: [-3.2, 1.45, -3.4] },
  { n: '厨房与餐厅', p: [3.1, 1.62, -3.6], t: [7.2, 1.35, 3.6] },
  { n: '楼梯厅与挑空', p: [2.15, 1.68, 4.15], t: [-0.55, 2.7, -0.4] },
  { n: '主卧', p: [2.5, 5.35, 6.3], t: [6.8, 4.6, 2.4] },
  { n: '屋顶露台', p: [-8.35, 5.05, -3.6], t: [-6.5, 4.15, 5.4] },
  { n: '细节 · 材料交接', p: [-6.05, 1.78, 8.75], t: [-8.7, 2.75, 5.1] },
  { n: '鸟瞰', p: [15, 23, 33], t: [-2.5, 2, 6.5] },
];
...
// 平面图卡片：点图纸收起，收起后右上角留一枚按钮把它叫回来 —— 收起就再也拿不
// 回来的东西，用户第一次点之前得先犹豫一下。
...
applyTOD(tod);
// 开场就站在正面。入口院落那一眼是叙事的起点，却是斜着看过去的半张脸；一进来
// 先给整张南立面 —— 玻璃、悬挑、露台一次看全，之后再自己走去别的机位。
